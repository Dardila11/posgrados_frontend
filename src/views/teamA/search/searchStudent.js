...
export const SearchStudent = ({ callback }) => {
  ...
  const getIdStudent = name => {
    let find;
    if (studentList) {
      find = studentList.find(student => student.user.name === name); // NOTA verificar el nombre del programa con el back (Program.name)
    }
  ...
  return (
    <Autocomplete
      ...
      options={studentList}
      getOptionLabel={option => option.user.username} // option name
